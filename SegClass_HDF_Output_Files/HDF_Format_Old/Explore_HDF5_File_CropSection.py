# TODO: Reconstruct Voronoi tesselation and calculate the density of a random cell in the centre of the field of view!
...
def GetXandYcoordinatesPerFrame(hdf5_file, frame=0):
    ...
    for object in list(f["frames"]["frame_{}".format(frame)]["coords"]):
        if float(object[1]) >= 300.0 and float(object[1]) < 650.0:
            if float(object[2]) >= 700.0 and float(object[2]) < 950.0:
                if int(object[4]) == 1:
...
PlotXandYcoordinatesPerFrame(hdf5_file=filename, frame=2)
vor, tri = DrawVoronoiTesselation(hdf5_file=filename, frame=2)
