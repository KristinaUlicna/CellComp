...
def check_whether_neighborhood_exists(winner_fn):
    """ Checks whether the file 'tracks_type1_nhood.xml' already exists or not.
        Returns its absolute directory.
        TODO: fn = file name?
    """
    winner_fn, ext = os.path.splitext(winner_fn)
    winner_fn += "_nhood.xml"
...
def calculate_neighborhood(winner_fn=None, loser_fn=None, d_thresh=100.0):
    ...
    Args:
        winner_fn, loser_fn (str) -> absolute directory to 'tracks_type1.xml' or 'tracks_type2.xml'
                                     if 100% population is analysed, supply the other channel as None
        d_thresh (float) -> homeostatic density of the analysed population; change as appropriate:
                            Pure populations:
                                MDCK_wildtype       = 100.0
                                MDCK_Sc_(kd)        = 133.0
                            Co-cultures:
                                ...
                                10% WT + 90% Sc     = 133.0
    Return:
        'tracks_type1_nhood.xml' & 'tracks_type2_nhood.xml' in the same directory as input files.
    """

    # TODO: Why are you checking whether the '_nhood.xml' file exists? Can you not just overwrite it?
