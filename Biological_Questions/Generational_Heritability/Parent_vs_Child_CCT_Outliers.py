...
def PlotParentChildCCTDiff_Outliers(left_or_right="left", show=False, print_stats=False):
    ...
    # Define figure saving directory
    merged_file = "/Volumes/lowegrp/Data/Kristina/MDCK_WT_Pure/cellIDdetails_merged.txt"
    directory = merged_file.split("/")[:-1]
    directory = "/".join(directory) + "/bulk_analysis/outliers_{}/".format(left_or_right)
    if not os.path.exists(directory):
        os.makedirs(directory)

    # Call the function to get the list of outliers:
    outlier_list = ShortlistOutliers(left_or_right=left_or_right)
    ...
    for outlier in outlier_list:
        # FindItself()
        child_info = FindFamily(cell_ID=outlier, filtered_file=merged_file).FindItself()
        child_list.append(child_info)
        # FindParent()
        parent_info = FindFamily(cell_ID=outlier, filtered_file=merged_file).FindParent()
        parent_list.append(parent_info)
        # FindSibling()
        sibling_info = FindFamily(cell_ID=outlier, filtered_file=merged_file).FindSibling()
    ...
    plt.title("Difference PARENT - CHILD; {} outliers\n({} cell pairs)".format(left_or_right, len(difference_list)))
    ...
    # Save, show & close:
    #plt.savefig(directory + "Hist_Parent_vs_Child_Diff_{}.jpeg".format(left_or_right), bbox_inches="tight")
    if show is True:
        plt.show()
    plt.close()
    ...
    plt.title("Ratio PARENT - CHILD; {} outliers\n({} cell pairs)".format(left_or_right, len(ratio_list)))
    plt.xlim(0, limit)
    plt.xlabel("Ratio")
    plt.ylim(-0.5)
    plt.ylabel("Cell_ID Pairs [#]")
    ...
    # Save, show & close:
    #plt.savefig(directory + "Hist_Parent_vs_Child_Ratio_{}.jpeg".format(left_or_right), bbox_inches="tight")
    if show is True:
        plt.show()
    plt.close()
    ...
    plt.title("Difference SIBLING 1 - SIBLING 2 (absolute value); {} outliers\n({} cell pairs - included twice)"
              .format(left_or_right, int(len(siblings) / 2)))
    plt.xlim(-1, limit + 1)
    plt.xlabel("Difference [hrs]")
    plt.ylim(-0.5)
    plt.ylabel("Cell_ID Pairs [#]")
    ...
    # Save, show & close:
    #plt.savefig(directory + "Hist_Sibling1_vs_Sibling2_Diff_{}.jpeg".format(left_or_right), bbox_inches="tight")
    if show is True:
        plt.show()
    plt.close()
    ...
    plt.axvline(x=1, color='grey', linestyle='dashed', linewidth=1.5)
    plt.title("Ratio SIBLING 1 - SIBLING 2; {} outliers\n({} cell pairs - included twice (inverse?)"
              .format(left_or_right, int(len(sibling_ratio_list) / 2)))
    plt.xlim(0, 2)
    plt.xlabel("Ratio")
    plt.ylim(-0.5)
    plt.ylabel("Cell_ID Pairs [#]")
    ...
    # Save, show & close:
    #plt.savefig(directory + "Hist_Sibling1_vs_Sibling2_Ratio_{}.jpeg".format(left_or_right), bbox_inches="tight")
    if show is True:
        plt.show()
    plt.close()
    """
...
PlotParentChildCCTDiff_Outliers(left_or_right="left", show=True, print_stats=True)
PlotParentChildCCTDiff_Outliers(left_or_right="middle", show=True, print_stats=True)
PlotParentChildCCTDiff_Outliers(left_or_right="right", show=True, print_stats=True)
